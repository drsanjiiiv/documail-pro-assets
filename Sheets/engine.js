  ...
  // ==========================================
  // FUNCTION: RESOLVE_DESTINATION_FOLDER
  // REQUIRED destination folder - no auto-creation.
  // Returns the folder ID string, or { error: message } when no folder is set.
  // ==========================================
  function RESOLVE_DESTINATION_FOLDER(payload) {
    if (payload && payload.folderDestination && typeof payload.folderDestination === "string" && payload.folderDestination !== "") {
      try {
        ...
        Logger.log("RESOLVE_DESTINATION_FOLDER parse error: " + e.message);
      }
    }
    return { error: "No destination folder selected. Please edit the template and choose a destination folder in Step 6 (Destination Folder), then try again." };
  }

  // ==========================================
  // FUNCTION: EXPORT_DOC_TO_PDF
  // Reliable file -> PDF blob export that works under the drive.file scope.
  // Uses UrlFetchApp to the Drive v3 REST endpoints with alt=media (the
  // Advanced Service export omits alt=media, which causes the "Export
  // requires alt=media" error). Google Workspace docs are exported to PDF;
  // native PDF files are downloaded directly.
  // ==========================================
  function EXPORT_DOC_TO_PDF(fileId) {
    var meta = Drive.Files.get(fileId, { fields: 'mimeType' });
    var url;
    if (meta && meta.mimeType && meta.mimeType.indexOf('application/vnd.google-apps.') === 0) {
      url = 'https://www.googleapis.com/drive/v3/files/' + encodeURIComponent(fileId) + '/export?mimeType=application/pdf&alt=media';
    } else {
    ...
    throw new Error('PDF export failed (' + response.getResponseCode() + '): ' + response.getContentText().substring(0, 500));
  }
  ...
  var displayValues = sheet.getRange(2, 1, lastRow - 1, allHeaders.length).getDisplayValues();
      ...
      // Get destination folder - REQUIRED, no auto-creation
      var destinationFolderId = RESOLVE_DESTINATION_FOLDER(payload);
      if (destinationFolderId && typeof destinationFolderId === "object" && destinationFolderId.error) {
        return destinationFolderId.error;
      }
      ...
      for (var i = 0; i < dataValues.length; i++) {
        ...
        var newDocFile = Drive.Files.copy({ name: fileName, parents: [destinationFolderId] }, templateId);
        var newDocId = newDocFile.id;
        ...
        var finalFileUrl = "https://docs.google.com/document/d/" + newDocId + "/edit";

        // Convert to PDF if format is PDF (export -> create PDF -> then trash the gdoc)
        if (payload.format === "PDF") {
          try {
            var pdfBlob = EXPORT_DOC_TO_PDF(newDocId);
            var pdfFileName = fileName + ".pdf";
            var pdfFile = Drive.Files.create({ name: pdfFileName, parents: [destinationFolderId] }, pdfBlob);
            try {
              Drive.Files.update({ trashed: true }, newDocId);
            } catch (trashErr) {
              Logger.log("Could not trash source gdoc " + newDocId + ": " + trashErr.message);
            }
            finalFileId = pdfFile.id;
            finalFileUrl = "https://drive.google.com/file/d/" + pdfFile.id + "/view";
          } catch (pdfErr) {
            Logger.log("PDF conversion failed for " + newDocId + ": " + pdfErr.message);
            throw new Error("PDF conversion failed: " + pdfErr.message);
          }
        }
        ...
        if (payload.isPreview) {
          return {
            ...
            message: mode + " Completed Successfully!\n\n✅ New Documents Created: " + processedCount +
              ...
              "\n📂 Saved to Destination Folder: " + destinationFolderId,
  ...
  function EXECUTE_DOCUMENT_MERGE_ENGINE_FOR_SINGLE_ROW(payload, singleRowData, rowNum, allHeaders, singleRowDisplayData, optSheet) {
    ...
    try {
      ...
      var destinationFolderId = RESOLVE_DESTINATION_FOLDER(payload);
      if (destinationFolderId && typeof destinationFolderId === "object" && destinationFolderId.error) {
        return { success: false, error: destinationFolderId.error };
      }
  ...
  // ============================================================================
  // REFRESH STALE TAG MAPPINGS IF THE LINKED DOC HAS CHANGED
  // Detects when a template record's saved tagMappings no longer match the
  // current document's tags (e.g. user re-pointed the template to a new doc)
  // and clears them so the run/preview path re-extracts from the live doc.
  // ============================================================================
  function REFRESH_TAG_MAPPINGS_IF_DOC_CHANGED(config) {
    if (!config || !config.tagMappings || Object.keys(config.tagMappings).length === 0) {
      return;
    }
    var savedTags = Object.keys(config.tagMappings);
    var liveTags = EXTRACT_TEMPLATE_TAGS_STREAM(config.templateUrl);
    var docChanged = savedTags.length !== liveTags.length;
    if (!docChanged) {
      for (var i = 0; i < savedTags.length; i++) {
        if (liveTags.indexOf(savedTags[i]) === -1) {
          docChanged = true;
          break;
        }
      }
  ...
  // ============================================================================
  function PREVIEW_TEMPLATE(templateId, previewFileUrl, previewFileName, previewError) {
    try {
      ...
      if (showDocumentSection) {
        ...
        if (fileUrl) {
          ...
          if (docMissing) {
            ...
            previewHtml += '<p style="margin:0; color:#c5221f;"><strong>❌ Preview Error</strong></p>';
            previewHtml += '<p style="margin:5px 0; color:#c5221f;">' + escapeHtml(previewError) + '</p>';
          } else {
  ...
  function RUN_TEMPLATE(templateId, optSheet, optTemplate) {
    ...
    if (template.type === "BOTH") {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (pdfFileId) {
          try {
            var pdfBlob = EXPORT_DOC_TO_PDF(pdfFileId);
      ...
      if (processedCount === 0 && emailSentCount === 0 && errors.length === 0) {
        return "NO_ROWS_ELIGIBLE";
      }
